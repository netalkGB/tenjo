...
export function AgentHistoryItem({
  title,
  id,
  skeleton = false,
  pinned = false
}: AgentHistoryItemProps) {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const { openDialog, closeDialog } = useDialog();
  const { togglePin, renameProject, deleteProject } = useAgentHistory();
  const { id: currentTaskId } = useParams<{ id: string }>();
  const [menuOpen, setMenuOpen] = useState(false);
  const testIdPrefix = 'sidebar-agent-history-item';
  ...
  const handleRename = (event: React.MouseEvent) => {
    ...
    const handleRenameSubmit = async () => {
      ...
      await renameProject(id, currentValueRef.value.trim());
  ...
  return (
    ...
    >
      <SidebarMenuSubButton asChild className="pr-8">
        <Link to={`/agent/task/${id}`} className="overflow-hidden">
